...
function Confirm-Parameters {
  ...
  $global:MachineType = $trimmedMachineType
  if ($global:MachineType.Length -eq 0) {
    Write-LogError "Unable to identify MachineType."
  }
}
#endregion
...
try {
  ...
  if ($MachineType.Length -eq 4) {
    $catalogUrl = "https://download.lenovo.com/catalog/$MachineType`_$global:OS.xml"
    $catalog = Get-XmlFile -Url $catalogUrl

    if (!$catalog) {
      Write-LogError "Failed to download the updates catalog from $catalogUrl. Check that $MachineType is a valid machine type."; Exit
    }
    ...
    if ($packagesCount -ne 0) {
      ...
      foreach ($item in $packages.GetEnumerator()) {
        ...
        if (($global:rt -contains $pkgXML.Package.Reboot.type) -and ($global:pt -contains $pkgXML.Package.PackageType.type)) {
          ...
          $sub13sub.SetAttribute("mtm", $MachineType)
    ...
    Write-LogWarning "Skipping $MachineType as it is not a valid machine type."
  }
